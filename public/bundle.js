...
var _graph = __webpack_require__(175);
...
exports.requestData = undefined;

var _list = __webpack_require__(174);

var StreamList = _interopRequireWildcard(_list);
  ...
  streams.onreadystatechange = function () {
    ...
    if (streams.readyState === DONE) {
      if (streams.status === OK) {
        ...
        results.streamData.forEach(function (obj) {
        ...
        games.onreadystatechange = function () {
          if (games.readyState === DONE) {
            if (games.status === OK) {
              games.response.data.forEach(function (game) {
              ...
              users.onreadystatechange = function () {
                if (users.readyState === DONE) {
                  if (users.status === OK) {
                    users.response.data.forEach(function (user) {
                    ...
                    var graph1Select = document.getElementById("graph1-select");
                    var graph2Select = document.getElementById("graph2-select");
...
exports.makeViewerBubbleGraph = exports.makeViewerBarGraph = exports.makeViewerPieChart = exports.makeGameBubbleGraph = exports.makeGamePieChart = exports.makeGameBarGraph = undefined;
...
var makeGameBarGraph = exports.makeGameBarGraph = function makeGameBarGraph(data) {
  ...
  var svg = d3.select("#graph1").append("svg").attr("height", height + margin).attr("width", width + margin);
  ...
  bar.on("mouseover", function (d) {
    ...
    d3.select(this).select("rect").transition().duration("500").attr("height", 3 * barHeight).attr("width", scale(d.count) + 10).attr("fill-opacity", 0.7);
  });

  bar.on("mouseout", function (d) {
    tooltip.style("display", "none");
    d3.select(this).select("rect").transition().duration("500").attr("height", barHeight).attr("width", scale(d.count)).attr("fill-opacity", 1.0);
  });
...
var makeGamePieChart = exports.makeGamePieChart = function makeGamePieChart(data) {
  ...
  path.on("mouseover", function (d) {
    ...
    d3.select(this).transition().duration("500").attr("d", overArc).attr("fill-opacity", 0.7);
  });

  path.on("mouseout", function (d) {
    tooltip.style("display", "none");

    d3.select(this).transition().duration("500").attr("d", arc).attr("fill-opacity", 1.0);
  });
...
var makeGameBubbleGraph = exports.makeGameBubbleGraph = function makeGameBubbleGraph(data) {
  var dims = d3.select("#graph1").node().getBoundingClientRect();
  var height = dims.height;
  var width = dims.width;
  var labelWidth = 0;
  ...
  var bubble = d3.pack({ children: data }).size([width, height]).padding(1.5);

  var nodes = d3.hierarchy({ children: data }).sum(function (d) {
    return d.count;
  });

  var bubbles = svg.selectAll(".bubble").attr("transform", "translate(0,0)").data(bubble(nodes).descendants()).enter().filter(function (d) {
    return !d.children;
  }).append("g");

  bubbles.append("circle").attr("class", "bubble").attr("cx", function (d) {
    return d.x;
  }).attr("cy", function (d) {
    return d.y;
  }).attr("r", function (d) {
...
var makeViewerPieChart = exports.makeViewerPieChart = function makeViewerPieChart(data, users) {
  ...
  path.on("mouseover", function (d) {
    ...
    d3.select(this).transition().duration("500").attr("d", overArc).attr("opacity", 0.7);
  });

  path.on("mouseout", function (d) {
    tooltip.style("display", "none");

    d3.select(this).transition().duration("500").attr("d", arc).attr("opacity", 1.0);
  });
...
var makeViewerBarGraph = exports.makeViewerBarGraph = function makeViewerBarGraph(data, users) {
  ...
  bar.on("mouseover", function (d) {
    ...
    d3.select(this).select("rect").transition().duration("500").attr("height", 3 * barHeight).attr("width", scale(d.viewer_count / 1000) + 10).attr("fill-opacity", 0.7);
  });

  bar.on("mouseout", function (d) {
    tooltip.style("display", "none");

    d3.select(this).select("rect").transition().duration("500").attr("height", barHeight).attr("width", scale(d.viewer_count / 1000)).attr("fill-opacity", 1.0);
  });
